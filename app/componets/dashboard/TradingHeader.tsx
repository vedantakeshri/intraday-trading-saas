...
export default function TradingHeader({ range, setRange }: TradingHeaderProps) {
  return (
    <div className="flex justify-between items-center bg-gray-900 border border-gray-800 rounded-xl p-3">
      
      {/* Symbol Selector */}
      <select className="bg-gray-800 px-3 py-1 rounded text-sm outline-none">
        <option value="NSE:NIFTY">NIFTY</option>
        <option value="NSE:BANKNIFTY">BANKNIFTY</option>
        <option value="NSE:FINNIFTY">FINNIFTY</option>
      </select>
      ...
      {/* Timeframe Filter */}
      <div className="flex gap-2">
        {(["1D", "5D", "15D", "60D"] as TimeRange[]).map((tf) => (
          <button
            key={tf}
            onClick={() => setRange(tf)}
